        ...
        <>
            ...
            <div className="flex flex-1 flex-col items-center justify-center px-4 py-10 sm:px-6">
                <div className="mx-auto max-w-xl text-center">
                    <span className="inline-flex items-center rounded-full bg-lavender-wash px-4 py-1 text-xs font-medium tracking-[0.004em] text-foreground sm:text-sm">
                        Tracking Penerbitan
                    </span>
                    <h1 className="mt-5 text-3xl font-semibold tracking-[0.016em] sm:mt-6 sm:text-4xl md:text-5xl">
                ...
                <form
                    onSubmit={submit}
                    className="mt-8 w-full max-w-lg space-y-5 rounded-xl border border-border bg-card p-5 shadow-sm sm:mt-10 sm:space-y-6 sm:p-6"
                >
                            ...
                            {METODE.map(({ key, label, keterangan }) => {
                                ...
                                return (
                                    <button
                                        ...
                                        aria-pressed={active}
                                        className={`relative flex flex-row items-center justify-between gap-1 rounded-lg border px-3 py-2.5 text-sm font-medium transition-all duration-150 active:scale-[0.98] xs:flex-col xs:justify-center xs:gap-0.5 sm:flex-col sm:gap-0.5 ${
                                            active
                                                ? 'border-cobalt-surface/40 bg-lavender-wash text-foreground ring-1 ring-cobalt-surface/30'
                                                : 'border-input bg-background hover:bg-accent'
                                        }`}
                                    >
                                        {active && (
                                            <Check className="size-4 text-primary xs:absolute xs:right-1.5 xs:top-1.5 xs:size-3.5 sm:absolute sm:right-1.5 sm:top-1.5 sm:size-3.5" />
                                        )}
                                        <span className="flex items-baseline gap-1.5 xs:flex-col xs:items-center xs:gap-0 sm:flex-col sm:items-center sm:gap-0">
                                            <span>{label}</span>
